...
description: >-
  ...
  set. Reads the active client's competitive-context resource for framing
  principle and brand-balance constraints; holds no client knowledge of its own.
license: SEE LICENSE IN LICENSE
compatibility: >-
  Requires benchmark-scores produced by benchmark-methodology and a configured
  client with a competitive-context resource that defines the strategic tension,
  target quadrant, and brand balance. Third and final step in the three-skill
  competitive pipeline.
metadata:
  skillforge.owner: platform
  skillforge.registryKey: competitive-report-structure
  skillforge.sourceKind: benchmark-scores
  skillforge.resultKind: competitive-report
---
...
Before assembling the report, read the active client's `competitive-context`
...
The whole report is organized around the client's strategic tension (from
competitive-context) and recommendations resolve back to the client's deliberate
brand balance. Recommendations that would break that balance must be flagged
against it explicitly — "this move shifts the balance from X/Y/Z toward A/B/C;
confirm intent."
...
- **White-space:** the position the client can own that rivals don't — argued
  ...
  (from competitive-context) is genuinely open.
...
brand sharpening). **Tie every recommendation back to the brand balance from
competitive-context** and flag any that would shift it. Sequence by impact ×
effort.
...
report auditable and defensible — carry the `deep-research` citation discipline
through.
...
- Is the target quadrant (from competitive-context) truly open, or is a rival
  already moving in?
